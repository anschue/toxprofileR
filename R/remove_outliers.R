...
remove_outliers <- function(nodeframe, plots = F){
    ...
    if(plots&log10(outliernew)< -3){plot(nodeframe$logFC,main=nodeframe$nodeID)}
    while(log10(outliernew)< -3){
        if(plots){points(x=which(nodeframe$logFC==outliers::outlier(nodeframe$logFC)),y=outliers::outlier(nodeframe$logFC),col="red")}
