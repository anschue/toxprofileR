...
enrich_clusters <- function(cluster_table){
    ...
    cluster_enrichments <- pbapply::pblapply(unique(cluster_table$clustername), function(cluster){
        ...
        ezfin <- clusterProfiler::enricher(gene = genes_ensembl,
        ...
        einterpro <- clusterProfiler::enricher(gene = genes_ensembl,
                                               universe = ensembl_universe,
        ...
        ereactome <- clusterProfiler::enricher(gene = genes_ensembl,
                                               universe = ensembl_universe,
        ...
        ego <- clusterProfiler::enrichGO(gene = genes_ensembl,
                                         universe = ensembl_universe,
                                         OrgDb         = "org.Dr.eg.db",
                                         keyType       = 'ENSEMBL',
                                         ont           = "all",
...
return(list(ZFIN = zfin_enrichments, interpro = interpro_enrichments, reactome = reactome_enrichments, GO = GO_enrichments))
    } else {message("For this function you need to install the package clusterProfiler")}
}
