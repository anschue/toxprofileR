...
map$EnsemblID<- unlist(lapply(strsplit(x = as.character(map$EnsemblID),split = ".",fixed = T),function(x){x[[1]]}))
map$allEnsemblIDs<-lapply(strsplit(as.character(map$allEnsemblIDs),split=";",fixed=T),function(x){unlist(lapply(strsplit(x,split = ".",fixed=T),function(y){y[[1]]}))})

map$countTranscriptID<-unlist(lapply(map$allEnsemblIDs,function(x){length(x)}))
...
map$wikigene_name<-lapply(map$gene_names,function(x){unique(x[,"wikigene_name"])})
map$count_wikigene<-unlist(lapply(map$gene_names,function(x){length(unique(x[,"wikigene_name"]))}))

map$entrezgene<-lapply(map$gene_names,function(x){unique(x[,"entrezgene"])})
map$count_entrezgene<-unlist(lapply(map$gene_names,function(x){length(unique(x[,"entrezgene"]))}))

map$ensembl_gene_id<-lapply(map$gene_names,function(x){unique(x[,"ensembl_gene_id"])})
map$count_ensemblgene<-unlist(lapply(map$gene_names,function(x){length(unique(x[,"ensembl_gene_id"]))}))

map$wikigene_description<-lapply(map$gene_names,function(x){unique(x[,"wikigene_description"])})
...
ncEnsembl<-unlist(lapply(strsplit(names(ncrna),split = ".",fixed = T),function(x){x[[1]]}))
